 ...
 */
package c.org.rajawali3d.objectpicking;
...
public class ObjectColorPicker implements IObjectPicker {
	...
	public ObjectColorPicker(SceneView sceneView) {
		mSceneView = sceneView;
		mSceneView.initializeColorPicker(this);
	}

	public void initialize() {
		final int size = Math.max(mSceneView.getViewportWidth(), mSceneView.getViewportHeight());
		...
		mSceneView.addRenderTarget(mRenderTarget);
	...
	public void getObjectAt(float x, float y) {
		if (mObjectPickedListener != null) {
			mSceneView.getCurrentScene().requestColorPicking(new ColorPickerInfo(x, y, this));
		}
	}
	...
	public static void pickObject(ColorPickerInfo pickerInfo) {
		...
		if (listener != null) {
			...
			GLES20.glReadPixels(pickerInfo.getX(),
					picker.mSceneView.getViewportHeight() - pickerInfo.getY(),

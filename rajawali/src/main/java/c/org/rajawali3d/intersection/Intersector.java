...
public final class Intersector {
    ...
    /**
     * Simple intersection of two Axis-Aligned Bounding Boxes. Edge touching is counted as intersection.
     *
     * @param boxA The first {@link AABB}
     * @param boxB The second {@link AABB}
     *
     * @return {@code int} If the two boxes touch or overlap anywhere.
     */
    @Intersection
    public static int intersect(@NonNull AABB boxA, @NonNull AABB boxB) {
        final Vector3 aMin = boxA.getMinBound();
        final Vector3 aMax = boxA.getMaxBound();
        final Vector3 bMin = boxB.getMinBound();
        final Vector3 bMax = boxB.getMaxBound();
        // First check if any intersection exists
        if (aMax.isGreaterThanEqual(bMin) && aMin.isLessThanEqual(bMax)) {
            // Either inside, or intersect
            if ((aMin.isLessThan(bMin) && aMax.isGreaterThan(bMax))
                || (bMin.isLessThan(aMin) && bMax.isGreaterThan(aMax))) {
    ...
    public static boolean intersectRayPlane(Vector3 rayStart, Vector3 rayEnd, Plane plane, Vector3 hitPoint) {
        ...
        if (denorm != 0) {
            double t = -(rayStart.dot(plane.getNormal()) + plane.getDistanceToOrigin()) / denorm;
            if (t < 0) {
                return false;
            }
            ...
            return true;
        } else if (plane.getPointSide(rayStart) == Plane.ON_PLANE) {
            if (hitPoint != null) {
     ...
     * @return True if there is an intersection, false otherwise.
     */
    public static boolean intersectRayTriangle(Vector3 rayStart, Vector3 rayEnd, Vector3 t1, Vector3 t2, Vector3 t3,
                                               Vector3 hitPoint) {
        Vector3 rayDir = Vector3.subtractAndCreate(rayEnd, rayStart);
        rayDir.normalize();
        p.set(t1, t2, t3);
        if (!intersectRayPlane(rayStart, rayEnd, p, i)) {
            return false;
        }
    ...
    /**
     * Intersects a ray defined by the start and end point and a sphere, returning the intersection point in
     * intersection.
     *
     * @param rayStart     Startpoint of the ray
     * @param rayEnd       Endpoint of the ray
     ...
     * @return True if there is an intersection, false otherwise.
     */
    public static boolean intersectRaySphere(Vector3 rayStart, Vector3 rayEnd, Vector3 sphereCenter,
                                             double sphereRadius, Vector3 hitPoint) {
        ...
        double c = Vector3.dot(sphereCenter, sphereCenter) + Vector3.dot(rayStart, rayStart) - 2.0f * Vector3
                .dot(sphereCenter, rayStart) - radius2;

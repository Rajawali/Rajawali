...
public class Etc1Texture2D extends CompressedTexture2D {
    ...
    public Etc1Texture2D(String textureName) {
        super(textureName);
        mCompressionType = Compression2D.ETC1;
        mCompressionFormat = ETC1.ETC1_RGB8_OES;
    }

    public Etc1Texture2D(int resourceId) {
        this(org.rajawali3d.materials.textures.TextureManager
                     .getInstance().getContext().getResources().getResourceName(resourceId));
        setResourceId(resourceId);
    }

    public Etc1Texture2D(String textureName, int resourceId, Bitmap fallbackTexture) {
        this(textureName);
    ...
    public Etc1Texture2D(String textureName, int[] resourceIds) {
        this(textureName);
        setResourceIds(resourceIds);
    }

    public Etc1Texture2D(String textureName, ByteBuffer byteBuffer) {
        this(textureName);
        setByteBuffer(byteBuffer);
    }

    public Etc1Texture2D(String textureName, ByteBuffer[] byteBuffers) {
        this(textureName);
        setByteBuffers(byteBuffers);
    }

    public Etc1Texture2D(String textureName, InputStream compressedTexture, Bitmap fallbackTexture) {
        this(textureName);

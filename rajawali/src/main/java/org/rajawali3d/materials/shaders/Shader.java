...
public abstract class Shader extends ShaderBase {
    ...
    private   Hashtable<String, String>    mPrecisionQualifier;
    ...
    public void initialize() {
        ...
        mPrecisionQualifier = new Hashtable<>();
    ...
    public void addPreprocessorDirective(String directive) {
        if (mPreprocessorDirectives == null) {
            mPreprocessorDirectives = new ArrayList<>();
        }
     ...
     *
     * @param dataType float, int or any sampler type available to the API level.
     * @param precision One of the valid choices from {@link Precision}.
     */
    protected void addPrecisionQualifier(String dataType, @Precision.Level String precision) {
        mPrecisionQualifier.put(dataType, precision);
    }

    /**
     * Add a uniform. The uniform qualifier is used to declare global variables whose values are the same across the
     * entire
    ...
    public ShaderVar clamp(ShaderVar var, float value1, float value2) {
        ShaderVar s = new ShaderVar(
                "clamp(" + var.getName() + ", " + Float.toString(value1) + ", " + Float.toString(value2) + ")",
                DataType.FLOAT);
        s.initialized = true;
        return s;
    }
    ...
    public ShaderVar mix(ShaderVar var1, ShaderVar var2, float value) {
        ShaderVar s = new ShaderVar(
                "mix(" + var1.getName() + ", " + var2.getName() + ", " + Float.toString(value) + ")", DataType.VEC3);
        s.initialized = true;
        return s;
    }

    public ShaderVar mix(ShaderVar var1, ShaderVar var2, ShaderVar var3) {
        ShaderVar s = new ShaderVar("mix(" + var1.getName() + ", " + var2.getName() + ", " + var3.getName() + ")",
                                    DataType.VEC3);
        s.initialized = true;
        return s;
    }
    ...
    public void startif(Condition... conditions) {
        ...
        for (int i = 0; i < conditions.length; i++) {
            ...
            if (i > 0) {
                shaderSB.append(condition.getJoinOperator().getOperatorString());
            }

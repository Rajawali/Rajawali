...
/**
 * NOTE: This class taken from Android Open Source Project source code and modified to support double precision
 * matrices.
 *
 * Matrix math utilities. These methods operate on OpenGL ES format matrices and vectors stored in double arrays.
 *
...
public final class Matrix {
    ...
    /**
     * Multiply two 4x4 matrices together and store the result in a third 4x4 matrix. In matrix notation:
     * result = lhs x rhs. Due to the way matrix multiplication works, the result matrix will have the same effect as
     * first multiplying by the rhs matrix, then multiplying by the lhs matrix. This is the opposite of what you
     * might expect.
     *
     * The same double array may be passed for result, lhs, and/or rhs. However, the result element values are
     * undefined if the result elements overlap either the lhs or rhs elements.
     *
     ...
     * notation: result = lhs x rhs
     *
     * The same double array may be passed for resultVec, lhsMat, and/or rhsVec. However, the resultVec element
     * values are undefined if the resultVec elements overlap either the lhsMat or rhsVec elements.
     *

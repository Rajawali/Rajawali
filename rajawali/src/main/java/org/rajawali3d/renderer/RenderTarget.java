...
import org.rajawali3d.GLNative;
...
public class RenderTarget {
     ...
     *
     * @param name          The name of the render target. This should be unique and should comply with regular
     *                         variable naming
     *                      standards.
     ...
     *
     * @param height The current texture height in pixels. Set the dimension to power of two unless NPOT extensions are
     *               supported.
     */
    ...
    /**
     * Resizes this render target. This method should be preferred over {@link #setWidth(int)} and
     * {@link #setHeight(int)}
     ...
     *
     * @param width The current texture width in pixels. Set the dimension to power of two unless NPOT extensions are
     *              supported.
     */
...
/*
	...
	*/
        ...
        if (status != GLES20.GL_FRAMEBUFFER_COMPLETE) {
            ...
            switch (status) {
                case GLES20.GL_FRAMEBUFFER_UNSUPPORTED:
                    errorString
                            = "GL_FRAMEBUFFER_UNSUPPORTED: returned if the combination of internal formats of the "
							  + "attached images violates an implementation-dependent set of restrictions.";
                    break;
                case GLES20.GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
                    errorString
                            = "GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT: returned if any of the framebuffer attachment "
    ...
    public void onGLError(String ex, int error) {
        if (error != GLES20.GL_NO_ERROR) {
            String description = GLU.gluErrorString(error);
            RajLog.e(ex + "[" + error + "]: " + description);
            throw new RuntimeException(ex + "[" + error + "]: " + description);

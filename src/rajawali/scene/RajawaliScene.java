...
public class RajawaliScene extends AFrameTask {
	...
	*/
	 ...
	 */
	private int getNumTriangle(BaseObject3D object) {
		int triangleCount = 0;
		...
		for (int i = 0, j = object.getNumChildren(); i < j; i++) {
			...
			if (child.getGeometry() != null && child.getGeometry().getVertices() != null && child.isVisible())
				if (child.getNumChildren()>0) {
					triangleCount+=getNumTriangle(child);
				} else {
	 ...
	 */
	private int getNumObjects(BaseObject3D object) {
		int objectCount = 0;
		...
		for (int i = 0, j = object.getNumChildren(); i < j; i++) {
			...
			if (child.getGeometry() != null && child.getGeometry().getVertices() != null && child.isVisible())
				if (child.getNumChildren()>0) {
					objectCount+=getNumObjects(child);
				} else {
					objectCount++;
				}
		}

 ...
 * 
 * It is intended that children, lights, cameras and animations will be added to this object and this object will be
 * added to the {@link RajawaliRenderer} instance.
 * 
...
public class RajawaliScene extends AFrameTask {
	...
	/**
	 * Temporary camera which will be switched to by the GL thread. Guarded by {@link #mNextSkyboxLock}
	 */
	...
	private SparseArray<ModelRunnable> mLoaderThreads;

	/**
	 * The camera currently in use. Not thread safe for speed, should only be used by GL thread (onDrawFrame() and
	 * render()) or prior to rendering such as initScene().
	 */
	protected Camera mCamera;
	private List<Camera> mCameras; // List of all cameras in the scene.
	/**
	 * Temporary camera which will be switched to by the GL thread. Guarded by {@link #mNextCameraLock}
	 */
	private Camera mNextCamera;
	private final Object mNextCameraLock = new Object();

	/**
	 * Frame task queue. Adding, removing or replacing members such as children, cameras, plugins, etc is now prohibited
	 * outside the use of this queue. The render thread will automatically handle the necessary operations at an
	 * appropriate time, ensuring thread safety and general correct operation.
	 * 
	 * Guarded by itself
	 */
	...
	/**
	 * Automatically creates the specified scene graph type with that graph's default behavior. If you want to use a
	 * specific constructor you will need to override this method.
	 */
	 ...
	 * 
	 * @param mCamera
	 *            {@link Camera} object to display the scene with.
	 */
	 ...
	 * 
	 * @param camera
	 *            Index of the {@link Camera} to use.
	 */
	...
	/**
	 * Fetches the {@link Camera} currently being used to display the scene. Note that the camera is not thread safe so
	 ...
	 * 
	 * @param camera
	 *            Index of the {@link Camera} to fetch.
	 ...
	 * 
	 * @param cameras
	 *            {@link Collection} of {@link Camera} objects to add.
	 * @return boolean True if the addition was successfully queued.
	 */
	...
	/**
	 * Removes a {@link Camera} from the scene. If the {@link Camera} being removed is the one in current use, the 0
	 * index {@link Camera} will be selected on the next frame.
	 * 
	 * @param camera
	 *            {@link Camera} object to remove.
	...
	/**
	 * Replaces a {@link Camera} in the renderer at the specified location in the list. This does not validate the
	 * index, so if it is not contained in the list already, an exception will be thrown.
	 * 
	 * If the {@link Camera} being replaced is the one in current use, the replacement will be selected on the next
	 * frame.
	 * 
	 * @param camera
	 *            {@link Camera} object to add.
	 * @param location
	 *            Integer index of the camera to replace.
	...
	/**
	 * Replaces the specified {@link Camera} in the renderer with the provided {@link Camera}. If the {@link Camera}
	 * being replaced is the one in current use, the replacement will be selected on the next frame.
	 * 
	 * @param oldCamera
	 *            {@link Camera} object to be replaced.
	 * @param newCamera
	 *            {@link Camera} object replacing the old.
	 * @param boolean True if the replacement was successfully queued.
	 */
	...
	/**
	 * Replaces a {@link Camera} at the specified index with an option to switch to it immediately.
	 * 
	 * @param camera
	 *            The {@link Camera} to add.
	 * @param location
	 *            The index of the camera to replace.
	...
	/**
	 * Replaces the specified {@link Camera} in the renderer with the provided {@link Camera}, switching immediately.
	 * 
	 * @param oldCamera
	 *            {@link Camera} object to be replaced.
	 * @param newCamera
	...
	/**
	 * Requests the addition of a child to the scene. The child will be added to the end of the list.
	 * 
	 * @param child
	 *            {@link Object3D} child to be added.
	 ...
	 * 
	 * @param children
	 *            {@link Collection} of {@link Object3D} children to add.
	 * @return boolean True if the addition was successfully queued.
	 */
	 ...
	 * 
	 * @param child
	 *            {@link Object3D} child to be removed.
	 ...
	 * 
	 * @param light
	 *            {@link ALight} child to be removed.
	 * @return boolean True if the child was successfully queued for removal.
	 */
	...
	/**
	 * Requests the addition of a plugin to the scene. The plugin will be added to the end of the list.
	 * 
	 * @param plugin
	 *            {@link Plugin} child to be added.
	 ...
	 * 
	 * @param plugin
	 *            {@link Plugin} child to be removed.
	...
	/**
	 * Register an animation to be managed by the scene. This is optional leaving open the possibility to manage updates
	 * on Animations in your own implementation.
	 * 
	 * @param anim
	 *            {@link Animation3D} to be registered.
	...
	/**
	 * Remove a managed animation. If the animation is not a member of the scene, nothing will happen.
	 * 
	 * @param anim
	 *            {@link Animation3D} to be unregistered.
	 ...
	 * 
	 * @param oldAnim
	 *            {@link Animation3D} the old animation.
	 * @param newAnim
	 *            {@link Animation3D} the new animation.
	 ...
	 * 
	 * @param anims
	 *            {@link Collection} containing the {@link Animation3D} objects to be added.
	 ...
	 * 
	 * @param resourceId
	 *            int Resouce id of the skybox texture.
	 ...
	 * @param negx
	 *            int Resource id for the right face.
	 * @param posy
	 *            int Resource id for the back face.
	 * @param negy
	 *            int Resource id for the left face.
	 * @param posz
	 *            int Resource id for the up face.
	 * @param negz
	 ...
	 * 
	 * @param resourceId
	 *            int the resource id of the new texture.
	 * @throws Exception
	 */
	...
	/**
	 * Applies the Rajawali default GL state to the driver. Developers who wish to change this default behavior can
	...
	public void render(double deltaTime, RenderTarget renderTarget) {
		...
		if (renderTarget != null)
		{
			...
			if (mReloadPickerInfo)
				pickerInfo.getPicker().reload();
	...
	/**
	 * Queue an addition task. The added object will be placed at the end of the renderer's list.
	 * 
	 * @param task
	 *            {@link AFrameTask} to be added.
	...
	/**
	 * Queue an addition task. The added object will be placed at the specified index in the renderer's list, or the end
	 * if out of range.
	 * 
	 * @param task
	 *            {@link AFrameTask} to be added.
	 * @param index
	 *            Integer index to place the object at.
	 * @return boolean True if the task was successfully queued.
	 */
	...
	/**
	 * Queue a removal task. The removal will occur at the specified index, or at the end of the list if out of range.
	 * 
	 * @param type
	 *            {@link AFrameTask.TYPE} Which list to remove from.
	 * @param index
	 *            Integer index to remove the object at.
	 * @return boolean True if the task was successfully queued.
	 */
	...
	/**
	 * Queue a replacement task to replace the object at the specified index with a new one. Replaces the object at the
	 * end of the list if index is out of range.
	 * 
	 * @param index
	 *            Integer index of the object to replace.
	 * @param replacement
	 *            {@link AFrameTask} the object replacing the old.
	 * @return boolean True if the task was successfully queued.
	 */
	 ...
	 * 
	 * @param task
	 *            {@link AFrameTask} the object to replace.
	 * @param replacement
	 *            {@link AFrameTask} the object replacing the old.
	 * @return boolean True if the task was successfully queued.
	 */
	 ...
	 * 
	 * @param collection
	 *            {@link Collection} containing all the objects to add.
	 * @return boolean True if the task was successfully queued.
	 */
	 ...
	 * 
	 * @param type
	 *            {@link AFrameTask.TYPE} Which object list to clear (Cameras, BaseObject3D, etc)
	 * @return boolean True if the task was successfully queued.
	 */
	...
	/**
	 * Queue a remove all task which will remove all objects from the given collection from the related list.
	 * 
	 * @param collection
	 *            {@link Collection} containing all the objects to be removed.
	 * @return boolean True if the task was successfully queued.
	 */
	 ...
	 * {@link #onModelLoadComplete(ALoader, int)}, {@link #onModelLoadFailed(ALoader, int, Exception)}, and
	 ...
	 * @return
	 */
	public ALoader loadModel(ALoader loader, int tag) {
		loader.setTag(tag);
		
		try {
			if (mLoaderThreads == null)
				mLoaderThreads = new SparseArray<ModelRunnable>();

			final ModelRunnable runnable = new ModelRunnable(loader, mLoaderThreads.size());

			mLoaderThreads.put(mLoaderThreads.size(), runnable);
			mLoaderExecutor.execute(runnable);
		} catch (Exception e) {
			onModelLoadFailed(loader);
		}
	 ...
	 * {@link #onModelLoadComplete(ALoader, int)}, {@link #onModelLoadFailed(ALoader, int, Exception)}, and
	 * {@link #onModelProgress(int, int)} to monitor the status of loading. Returns null if the loader fails to
	 * instantiate, {@link #onModelLoadFailed(ALoader, Exception)} will still be called. A tag will be set automatically
	 * for the model equal to the resource ID passed.
	 * 
	 * @param loaderClass
	 * @param resID
	 * @return
	 */
	public ALoader loadModel(Class<? extends ALoader> loaderClass, int resID) {
		return loadModel(loaderClass, resID, resID);
	}
	 ...
	 * {@link #onModelLoadComplete(ALoader, int)}, {@link #onModelLoadFailed(ALoader, int, Exception)}, and
	 * {@link #onModelProgress(int, int)} to monitor the status of loading. Returns null if the loader fails to
	 * instantiate, {@link #onModelLoadFailed(ALoader, Exception)} will still be called. Use the tag identified to
	 ...
	 * @return
	 */
	public ALoader loadModel(Class<? extends ALoader> loaderClass, int resID, int tag) {
		try {
	 ...
	 * @param loader
	 */
	protected void onModelLoadComplete(ALoader loader) {
		RajLog.d("Completed load of model: " + loader);
	}
	 ...
	 * @param loader
	 */
	protected void onModelLoadFailed(ALoader loader) {
		RajLog.e("Failed to load model: " + loader);
	}
	 ...
	 * 
	 * @param task
	 *            {@link AFrameTask} object to process.
	 */
	...
	/**
	 * Internal method for replacing a {@link Animation3D} object. If index is {@link AFrameTask.UNUSED_INDEX} then it
	 * will be used, otherwise the replace object is used. Should only be called through
	 * {@link #handleAddTask(AFrameTask)}
	 * 
	 * @param anim
	 *            {@link AFrameTask} The old animation.
	 * @param replace
	 *            {@link Animation3D} The animation replacing the old animation.
	 * @param index
	 *            integer index to effect. Set to {@link AFrameTask.UNUSED_INDEX} if not used.
	 */
	...
	/**
	 * Internal method for adding {@link Animation3D} objects. Should only be called through
	 * {@link #handleAddTask(AFrameTask)}
	 * 
	 * This takes an index for the addition, but it is pretty meaningless.
	 * 
	 * @param anim
	 *            {@link Animation3D} to add.
	...
	/**
	 * Internal method for removing {@link Animation3D} objects. Should only be called through
	 * {@link #handleRemoveTask(AFrameTask)}
	 * 
	 * This takes an index for the removal.
	 * 
	 * @param anim
	 *            {@link Animation3D} to remove. If index is used, this is ignored.
	 * @param index
	 *            integer index to remove the child at.
	 */
	...
	/**
	 * Internal method for removing all {@link Animation3D} objects. Should only be called through
	 * {@link #handleRemoveAllTask(AFrameTask)}
	 */
	...
	/**
	 * Internal method for replacing a {@link Camera}. If index is {@link AFrameTask.UNUSED_INDEX} then it will be used,
	 * otherwise the replace object is used. Should only be called through {@link #handleReplaceTask(AFrameTask)}
	 * 
	 * @param camera
	 *            {@link Camera} The old camera.
	 * @param replace
	 *            {@link Camera} The camera replacing the old camera.
	 * @param index
	 *            integer index to effect. Set to {@link AFrameTask.UNUSED_INDEX} if not used.
	 */
	...
	/**
	 * Internal method for adding a {@link Camera}. Should only be called through {@link #handleAddTask(AFrameTask)}
	 * 
	 * This takes an index for the addition, but it is pretty meaningless.
	 * 
	 * @param camera
	...
	/**
	 * Internal method for removing a {@link Camera}. Should only be called through
	 * {@link #handleRemoveTask(AFrameTask)}
	 * 
	 * This takes an index for the removal.
	 * 
	 * NOTE: If there is only one camera and it is removed, bad things will happen.
	 * 
	 * @param camera
	 *            {@link Camera} to remove. If index is used, this is ignored.
	 * @param index
	 *            integer index to remove the camera at.
	 */
	...
	/**
	 * Internal method for removing all {@link Camera} from the camera list. Should only be called through
	 * {@link #handleRemoveAllTask(AFrameTask)} Note that this will re-add the current camera.
	 */
	...
	/**
	 * Internal method for replacing a {@link ALightD} light. If index is {@link AFrameTask.UNUSED_INDEX} then it will
	 * be used, otherwise the replace object is used. Should only be called through
	 * {@link #handleReplaceTask(AFrameTask)}
	 * 
	 * @param light
	 *            {@link ALight} The new light for the specified index.
	 * @param replace
	 *            {@link ALight} The light replacing the old light.
	 * @param index
	 *            integer index to effect. Set to {@link AFrameTask.UNUSED_INDEX} if not used.
	 */
	...
	/**
	 * Internal method for adding a {@link ALight}. Should only be called through {@link #handleAddTask(AFrameTask)}
	 * 
	 * This takes an index for the addition, but it is pretty meaningless.
	 * 
	 * @param light
	...
	/**
	 * Internal method for removing a {@link ALight}. Should only be called through
	 * {@link #handleRemoveTask(AFrameTask)}
	 * 
	 * This takes an index for the removal.
	 * 
	 * NOTE: If there is only one light and it is removed, bad things will happen.
	 * 
	 * @param light
	 *            {@link ALight} to remove. If index is used, this is ignored.
	 * @param index
	 *            integer index to remove the light at.
	 */
	...
	/**
	 * Internal method for removing all {@link ALight} from the light list. Should only be called through
	 * {@link #handleRemoveAllTask(AFrameTask)} Note that this will re-add the current light.
	 */
	...
	/**
	 * Set the lights on all materials used in this scene. This method should only be called when the lights collection
	 * is dirty. It will trigger compilation of all light-enabled shaders.
	 */
	...
	/**
	 * Update the lights on this child's material. This method should only be called when the lights collection is
	 * dirty. It will trigger compilation of all light-enabled shaders.
	 * 
	 * @param child
	 */
	...
	/**
	 * Internal method for replacing a {@link Object3D} child. If index is {@link AFrameTask.UNUSED_INDEX} then it will
	 * be used, otherwise the replace object is used. Should only be called through
	 * {@link #handleReplaceTask(AFrameTask)}
	 * 
	 * @param child
	 *            {@link Object3D} The new child for the specified index.
	 * @param replace
	 *            {@link Object3D} The child replacing the old child.
	 * @param index
	 *            integer index to effect. Set to {@link AFrameTask.UNUSED_INDEX} if not used.
	 */
	...
	/**
	 * Internal method for adding {@link Object3D} children. Should only be called through
	 * {@link #handleAddTask(AFrameTask)}
	 * 
	 * This takes an index for the addition, but it is pretty meaningless.
	 * 
	 * @param child
	 *            {@link Object3D} to add.
	...
	/**
	 * Internal method for removing {@link Object3D} children. Should only be called through
	 * {@link #handleRemoveTask(AFrameTask)}
	 * 
	 * This takes an index for the removal.
	 * 
	 * @param child
	 *            {@link Object3D} to remove. If index is used, this is ignored.
	 * @param index
	 *            integer index to remove the child at.
	 */
	...
	/**
	 * Internal method for removing all {@link Object3D} children. Should only be called through
	 * {@link #handleRemoveAllTask(AFrameTask)}
	 */
	...
	/**
	 * Internal method for replacing a {@link IRendererPlugin}. If index is {@link AFrameTask.UNUSED_INDEX} then it will
	 * be used, otherwise the replace object is used. Should only be called through
	 * {@link #handleReplaceTask(AFrameTask)}
	 * 
	 * @param plugin
	 *            {@link IRendererPlugin} The new plugin for the specified index.
	 * @param replace
	 *            {@link IRendererPlugin} The plugin replacing the old plugin.
	 * @param index
	 *            integer index to effect. Set to {@link AFrameTask.UNUSED_INDEX} if not used.
	 */
	...
	/**
	 * Internal method for adding {@link IRendererPlugin} renderer. Should only be called through
	 * {@link #handleAddTask(AFrameTask)}
	 * 
	 * This takes an index for the addition, but it is pretty meaningless.
	 * 
	 * @param plugin
	 *            {@link IRendererPlugin} to add.
	 * @param int index to add the child at.
	 */
	...
	/**
	 * Internal method for removing {@link IRendererPlugin} renderer. Should only be called through
	 * {@link #handleRemoveTask(AFrameTask)}
	 * 
	 * This takes an index for the removal.
	 * 
	 * @param plugin
	 *            {@link IRendererPlugin} to remove. If index is used, this is ignored.
	 * @param index
	 *            integer index to remove the child at.
	 */
	...
	/**
	 * Internal method for removing all {@link IRendererPlugin} renderers. Should only be called through
	 * {@link #handleRemoveAllTask(AFrameTask)}
	 */
	 ...
	 * 
	 * @param plugin
	 *            {@link IRendererPlugin} to check for.
	...
	/**
	 * Clears any references the scene is holding for its contents. This does not clear the items themselves as they may
	 * be held by some other scene.
	 */
	 ...
	 * 
	 * @param red
	 *            float red component (0-1.0f).
	 * @param green
	 *            float green component (0-1.0f).
	 * @param blue
	 *            float blue component (0-1.0f).
	 * @param alpha
	 *            float alpha component (0-1.0f).
	 */
	...
	/**
	 * Indicate that the color buffer should be cleared on every frame. This is set to true by default. Reasons for
	 * settings this to false might be integration with augmented reality frameworks or other OpenGL based renderers.
	 * 
	 * @param value
	 */
	 ...
	 * 
	 * @param width
	 *            int the new viewport width in pixels.
	 * @param height
	 *            in the new viewport height in pixes.
	 */
	...
	/**
	 * Set if the scene graph should be displayed. How it is displayed is left to the implimentation of the graph.
	 * 
	 * @param display
	...
	private final Handler mLoaderHandler = new Handler(Looper.getMainLooper()) {
		...
		public void handleMessage(Message msg) {
			...
			final ALoader loader = mLoaderThreads.get(msg.arg2).mLoader;
